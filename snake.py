...
class Snake(PyGameWrapper):
    ...
    def getActualHeadPosition(self,direction):

        head_x = self.player.head.rect.centerx
        head_y = self.player.head.rect.centery
        head_width = self.player.head.image.get_width()
        head_height = self.player.head.image.get_height()

        # right
        if direction == "dreapta":
            return head_x + head_width, head_y + head_height // 2
        # left
        elif direction == "stanga":
            return head_x, head_y + head_height // 2
        # up
        elif direction == "sus":
            return head_x + head_width // 2, head_y
        # down
        elif direction == "jos":
            return head_x + head_width // 2, head_y + head_height

    def getHeadSize(self):
        return self.player.head.image.get_width(),self.player.head.image.get_height()

    def getFoodColor(self):
        return self.food.color
